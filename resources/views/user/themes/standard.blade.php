...
<div id="container">
    ...
    <div class="row">
        <div class="d-flex align-items-center mt-1 mx-auto"
            style="min-width:80px; border: 1px solid lightgrey; background: rgba(255, 255, 255, 0.1); border-radius: 50px;">
            ...
            <span style="overflow:hidden;" class="copyText mx-3">{{ route('public', $user->username) }}</span>
    ...
    " src="{{ asset($user->photo) }}" alt="">
            ...
            <div class="col-md-7 d-flex align-items-center">
                <div class="mx-auto">
                    <h3 class="bios-fullname text-center">{{ $user->fullname }}</h3>
                    @if ($user->location)

                        <div class="d-flex justify-content-center">
                    <span class="p-1 px-2"
                        style="display:inline-block; border-radius: 25px; color: #4e73df; border: 2px solid #4e73df">
                        <i class="fas fa-map-marker-alt" style="color: #4e73df"></i>
                        {{ $user->location }}</span>
                    </div>
                    @endif

                    <h6 class="py-4 text-center">{{ $user->bio }}</h6>
                </div>
            </div>
        </div>
    </div>
    <div class="link-buttons mx-auto mt-5 p-3">
        ...
        <div class="row mt-4">
            ...
            <a href="{{ $link->brand == 'mail' ? 'mailto:' : '' }}{{ $link->link }}" data-id="{{ $link->id }}"
                ...
                style="border-radius: 13px; padding: 0; border:none"
                class="btn d-flex align-items-center mt-3 btn-block btn-lg">
                <span style="border-right: 1px solid lightgray; padding: 16px;"
                ...
                <span id="link-text-{{ $loop->iteration }}"
                    class="text-light link-name mx-auto">{{ $link->name }}</span>
            </a>
            @endforeach
        </div>
    </div>
